...
public class ForecastActivity extends AppCompatActivity implements ApiCallback {
    ...
    private void setupDetailsPager() {
        ...
        if(screenSize == Configuration.SCREENLAYOUT_SIZE_LARGE || screenSize == Configuration.SCREENLAYOUT_SIZE_XLARGE){
            ...
            compositePageTransformer.addTransformer(new HorizontalPageTransformer());
            detailsPager.setPageTransformer(compositePageTransformer);
        }

        detailsPager.setAdapter(detailsAdapter);
    }

    public class HorizontalPageTransformer implements ViewPager2.PageTransformer {
        @Override
        public void transformPage(@NonNull View page, float position) {
            int pageWidth = page.getWidth();
            int offset = (int) (position * -(2 * pageWidth));
            if (ViewPager2.ORIENTATION_HORIZONTAL == 1) {
